...
$key = "userid";
$tableName = "e_admin";
$fields = array('Name','Username','Email','Role','Image','Status');
$indexes  = array('name','username','email','role','image','status');
 ?>
						...
						<?php foreach ($data as $row):?>
                        	<tr id="row<?=$row[$key]?>">
                        		<?php foreach ($indexes as $index): 
									if($index == 'image')
										$row[$index] = '<img class="img-responsive" src="'.base_url().'assets/img/'.$row[$index].'" style="width:50px">';
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>

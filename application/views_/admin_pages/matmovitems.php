...
$heading 		= "MatMov - Items";
$key 			= "id";
$tableName 		= "e_items";
$fields 		= array('MM #','Subject/Title','Status');//,'Parent Category'
$indexes  		= array('id','mm_title','approved_status');//,'parentid'
      ...
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          <div class="card-head">
            ...
            <div class="tools col-md-8"> Subject :<br>
              <?=$bom['mm_title']?>
            </div>
...
#saveXStage tr th {padding:0px; text-align:center; height:20px}
            ...
            <form name="myForm" id="myForm" method="post" enctype="multipart/form-data"  onsubmit="return form_validation()">
              ...
              <input type="hidden" value="e_mmitems" id="tableName" name="tableName">
		  ...
		  if($bomi){
              ...
              <td width="10%"><?=$item_id?></td>
                <td width="35%"><span onclick="idetailsToolTip(<?=$item_id?>)" onclcik="clearToolTip(<?=$item_id?>)">
                ...
                <button class="btn btn-primary btn-xs" onclick="updateitem(<?=$id?>,'e_mmitems')" > <i class="fa fa-pencil"></i></button>
                 
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  <button class="btn btn-danger btn-xs" onclick="deletBomItem('id',<?=$id?>,'e_mmitems')"> <i class="fa fa-trash-o "></i></button>
                  <?php endif?>
                  </td>
              </tr>
              <?php }
...
{   var tableName 	= 'e_mm';
	...
	if(confirm("Are you sure to prceed?")){
		...
		function(data)
		{  	
			window.location.href = "<?=base_url()?>Home/mmov";
		});
	}
}

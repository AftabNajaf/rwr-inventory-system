...
$heading 	= "Vaccinations  Data - VT";
$key 		= "id";
$tableName = "e_vaccinations";
$fields = array('ID Type','Citizen No.','Passport No OR Family Member AC/CNIC','Name','Father Name','Citizen Mobile No.','Gender','Age (Years)','Health Unit ID.','Location','Vaccinator Mobile No.','Vaccination Date.','Disability','Socio Economic Status','Vaccine','Vaccination ID.','Dosage','District','Team Id');
$indexes  = array('idtype', 'citizenid', 'passportno','name', 'fathername', 'mobileno', 'gender', 'age', 'healthunitid', 'location', 'vaccinatorno', 'vaccine_date', 'disability', 'sociostatus', 'vaccineid', 'batchno', 'dosage','district','teamid');
 ?>
        ...
        <div class="card card-topline-yellow">
          <div class="card-head">
           ...
           <div class="form-group col-md-2">
            ...
            <option value=""></option>
          	<?= getOption('e_locations','id','title')?>
            </select>
           </div>
           <div class="form-group col-md-2">
						...
						<?php /* foreach ($data as $row):?>
                        	<tr id="row<?=$row[$key]?>">
                        		<?php foreach ($indexes as $index): 
									...
									if($index == 'teamid')
										$row[$index] = getField('e_admin',array('userid'=>$row[$index]),'name');
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>
...
function loadData(){  
    ...
    $('#saveStage').DataTable( {
		...
		"ajax": {
                        "type": "GET",
                        "url": "<?=base_url();?>Home/dailyVTData",
						"data":{'location':location,'date_from':date_from,'date_to':date_to,'mobileno':mobileno,'citizenid':citizenid}
                    }

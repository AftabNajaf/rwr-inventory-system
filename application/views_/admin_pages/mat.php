...
$key = "id";
$tableName = "e_items";
$fields = array('MO #','Department','Section','Project','Generated On','Gen. By','Status');//,'Parent Category','Store Remarks'
$indexes  = array('id','department_id','section_id','project_id','generated_on','generated_by','approved_status');//,'parentid','bom_remarks'
...
<div class="page-content-wrapper">
      ...
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          <div class="card-head">
			...
			if(strpos($privileges,'matcreate')!==false):?>
              ...
              <tbody>
						<?php  foreach ($boms as $row):?>
                        	<tr id="row<?=$row[$key]?>">
                                ...
                                <?php if($this->session->role!='admin' && $row['approved_status']!='Draft') ://&& $row['approved_status']=='Approved'?>
                                		<?php if($this->session->role=='level2' && $row['approved_status']=='Submitted')  :?> 
                                        <button class="btn btn-success btn-xs" onclick="location.href = '<?=base_url()?>Home/matitems/<?=$row[$key]?>'" >
                                <i class="fa fa-plus"> Items</i></button>                                                               
                                		<button class="btn btn-primary btn-xs" onclick="addbom('edit',<?=$row[$key]?>)" >
                                <i class="fa fa-pencil"></i></button>
                                		<button class="btn btn-danger btn-xs" onclick="deleteRecordx('<?=$key?>',<?=$row[$key]?>,'e_mat')">
  ...
  '_blank')" >
                                ...
                                <?php if($row['store_remarks']!='' && $row['bom_remarks'] =='' && $this->session->role=='level1'  ) :?>
                                    <button class="btn btn-primary btn-xs" onclick="setReceivings('e_mat','<?=$row['generated_by']?>','id',<?=$row[$key]?>)" >
                                <i class="fa fa-gear"> [<?=$row['store_remarks']?>]</i></button>
                                    <?php endif?>

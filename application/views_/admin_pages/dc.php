...
$key = "id";
$tableName = "e_items";
$fields = array('DC #','DC Date','Sale No','Customer','Carrier','Generated On','Status');//,'Parent Category'
$indexes  = array('id','dc_date','sale_order_no','customer_id','carrier_name','generated_on','approved_status');//,'parentid'
...
<div class="page-content-wrapper">
      ...
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          <div class="card-head">
            ...
            <?php  if(strpos($privileges,'dccreate')!==false):?>
              ...
              <tbody>
						<?php foreach ($boms as $row):?>
                        	<tr id="row<?=$row[$key]?>">
								...
								if(($index=='customer_id')&& $row[$index]>0)
									$row[$index]= getField('e_customers',array('id'=>$row[$index]),'title');
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>
  ...
  '<?=base_url()?>Pdfgenerator/deliveryChallan/<?=$row[$key]?>',
  '_blank')" >
                                <i class="fa fa-eye"></i></button>
                                <?php else:?>
                                <button class="btn btn-success btn-xs" onclick="location.href='<?=base_url()?>Home/dcitems/<?=$row[$key]?>'">
  ...
  '_blank')" >
                                ...
                                <i class="fa fa-pencil"></i></button>
                                 ...
                                 <button class="btn btn-danger btn-xs" onclick="deleteRecordx('<?=$key?>',<?=$row[$key]?>,'e_dc')">
...
{   var viewType 	= 'DelChallan';
	var tableName 	= 'e_dc';
	var key 		= 'id'; 
	$("#exTitle").html(action+' Release Items');

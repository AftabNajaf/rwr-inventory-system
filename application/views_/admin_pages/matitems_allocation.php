    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          ...
          DEPT / SEC :<br><?=getField('e_departments',array('id'=>$bom['department_id']),'title')?> / <?=getField('e_departments',array('id'=>$bom['section_id']),'title')?> 
          </div>
                    
          <div class="tools col-md-5">
          PROJECT :<br><?=getField('e_projects',array('id'=>$bom['project_id']),'title')?>
          </div>
          <div class="col-md-2">
           <b>MAT No : <?=$bom['id']?></b><br>
		  ...
		  <style>#saveXStage tr th {padding:0px; text-align:center; height:20px}</style>    
            <table id="saveXStage" class="table table-bordered display full-width" >
                ...
                <th width="10%">
               <button class="btn btn-info btn-xs" onclick="setStoreRemarks(<?=$bom['id']?>,'mat','e_matitem','e_mat',<?=$bom['id']?>,'mat_id')" >Mark as Done</button>
                </th></tr>
		  ...
		  if($bomi){
			  foreach($bomi as $bi){ extract($bi);?>
                ...
                <td width="20%"><span onclick="idetailsToolTip(<?=$item_id?>)" onclick="clearToolTip(<?=$item_id?>)"><?=$item_id.'-'.getField('e_items',array("id"=>$item_id),'item_name');?></span> </td>
                <td width="10%"><?=$qty_needed?></td>
                <td width="10%"><?=getField('e_items_alloc',array("instrument_no"=>$bom['id'],"item_id"=>$item_id),'item_quantity');?></td>
                ...
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  ...
                  <?php endif?>
                 <button class="btn btn-success btn-xs" onclick="addEditItemAlloc('add',<?=$bom['id']?>,<?=$item_id?>,'mat')" > <i class="fa fa-gear"></i>Alloc. Item</button>

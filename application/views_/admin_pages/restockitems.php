...
<?php
 $instm='';$tblx='';
			  if($bom['instrument']=='MATS'){
			 	 	$bomix = $this->AdminModel->getRows('e_matitem',array('mat_id'=>$bom['instrument_no']));
					$instm='mat';
					$tblx='e_matitem';
			  }
			  else{
			  		$bomix = $this->AdminModel->getRows('e_tempissueitems',array('adv_id'=>$bom['instrument_no']));
					$instm='tmpissue';
					$tblx='e_tempissueitems';
			  }?>
            <form name="myForm" id="myForm" method="post" enctype="multipart/form-data"  onsubmit="return form_validation()">
              ...
              <input type="hidden" value="e_restockitems" id="tableName" name="tableName">
              ...
              <table id="saveXStage" class="table table-bordered display full-width" >
                ...
                <tr>
                  ...
                  <td><input type="text" class="form-control" name="item_id" id="item_id" <?php if($bom['instrument']!='MISNO') echo 'readonly';?> required ></td>
                  <td><!--<span  class="form-control" name="itm_display" id="itm_display" onclick="searchSelectItem()">Select Item</span>--><input type="text" class="form-control" name="itm_display" id="itm_display" disabled >
                    </td>
		...
		?>
          ...
          if($bomix){
              ...
              <tr id="row<?=$id?>">
                ...
                <td width="15%"><?=$item_notes?></td>
                <td><button class="btn btn-info btn-xs" onclick="fetchItem(<?=$id?>,'<?=$tblx?>')" > <i class="fa fa-gearx">Mark for<br /> Restock</i></button>
                  </td>
              </tr>
              <?php }
		  }
...
{  // alert(id+"___"+tableName);
	var itemid 	= id;
		$.post("<?php echo base_url(); ?>Function_control/fetchItemtoRestock",{tableName,id}, 
		function(data)
		{  // alert(data);
			var obj = JSON.parse( data ); 
			$("#qty_needed").val(obj.qty_needed);
			...
			$("#id").val(obj.id);
			$("#action").val('add');
			$("#itm_display").val(obj.item_id+" - "+obj.ititle);
			
			$("#qty_restocked").attr("max", obj.qty_limit);
		});

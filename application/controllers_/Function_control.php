...
class Function_control extends CI_Controller {
	...
	public function getAvailQty(){
		...
		$catz = $this->AdminModel->myQueryx("SELECT i.id,i.item_name,i.item_functionality,i.part_number, i.item_serial_no,i.image,i.doc_list,i.item_description,i.supplier_id, i.supplier_ref,i.item_category, i.item_addl_remarks FROM e_items i where  i.id=".$value);
	...
	if($catz){
		 extract($catz);
		  ...
		  $txt .="<div style='border-bottom: #ddd solid 1px'><strong>Supplier  : </strong>".getField("e_suppliers",array("id"=>$supplier_id),"title")."</div><br/>";
		if($catzQ){
		 extract($catzQ); 
	...
	public function getItemDetail(){
		$value = $this->input->post('itemID');
		$catz = $this->AdminModel->myQueryx("SELECT i.id,i.item_name,i.item_functionality,i.part_number, i.item_asset_no,i.item_serial_no,i.image,i.doc_list,i.item_description,i.supplier_id, i.supplier_ref,i.item_category, i.item_addl_remarks,i.weblink FROM e_items i where  i.id=".$value);
		$this->db->query('SET SESSION SQL_MODE=""');
		$catzQ = $this->AdminModel->myQueryx("SELECT sum(q.item_quantity) item_quantity, location, custodian FROM  e_items_qty q where q.item_id=".$value." order by id DESC");
...
$txt .="<div style='border-bottom: #ddd solid 1px'><strong>URL  : </strong><br /> <a href='".$weblink."' target='_blank'>Click Here</a></div><br /><br/>";

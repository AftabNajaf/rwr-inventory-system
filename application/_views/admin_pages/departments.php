...
$calledName = "Departments";
$heading = "Departments";
$key = "id";
$tableName = "e_departments";
$fields = array('Department Title','Parent Department');
            ...
            <table id="saveStage" class="table-bordered display full-width" >
              ...
              <tbody>
						<?php foreach ($data as $row):?>
							...
							if($index=='parentid' && $row[$index]>0) $row[$index]= getField('e_departments',array('id'=>$row[$index]),'title');
							else if($index=='parentid' && $row[$index]==0)$row[$index]='';
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>

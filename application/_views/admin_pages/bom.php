...
$key = "id";
$tableName = "e_items";
$fields = array('BOM #','Department','Section','Project','Generated On','Gen. By','Status');//,'Parent Category'
...
<div class="page-content-wrapper">
      ...
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          <div class="card-head">
			...
			if(strpos($privileges,'bomcreate')!==false):?>
              ...
              <tbody>
						<?php foreach ($boms as $row):?>
                        	<tr id="row<?=$row[$key]?>">
                                ...
                                <td style="padding: 2px;text-align: left;" width="15%">
                                ...
                                <?php if($this->session->role!='admin' && $row['approved_status']!='Draft') ://&& $row['approved_status']=='Approved'?>
                                		...
                                		<button class="btn btn-success btn-xs" onclick="location.href = '<?=base_url()?>Home/bomitems/<?=$row[$key]?>'" >
                                <i class="fa fa-plus"> Items</i></button>
                                		<button class="btn btn-primary btn-xs" onclick="addbom('edit',<?=$row[$key]?>)" >
                                <i class="fa fa-pencil"></i></button>
                                		<button class="btn btn-danger btn-xs" onclick="deleteRecordx('<?=$key?>',<?=$row[$key]?>,'e_bom')">
  ...
  '<?=base_url()?>Pdfgenerator/downloadBom/<?=$row[$key]?>',
  '_blank')" >
                                <i class="fa fa-eye"></i></button>
                                		<?php if( $row['approved_status']=='Approved') : //$this->session->role=='level2' &&?>
                                            <button class="btn btn-info btn-xs" onclick="submitBomAsMat(<?=$row['id']?>)" >
                                            <i class="fa fa-web">Submit as MAT</i></button>
                                		<?php endif?>
                                <?php else:?>
...
function submitBomAsMat(val)
{   
	if(confirm("Are you sure to prceed?")){
		$.post("<?php echo base_url(); ?>Function_control/submitBomAsMat",{val}, 
		function(data)
		{  	//alert(data);

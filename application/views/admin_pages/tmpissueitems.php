...
#saveXStage tr th {padding:0px; text-align:center; height:20px}
            ...
            <form name="myForm" id="myForm" method="post" enctype="multipart/form-data"  onsubmit="return form_validation()">
              ...
              <input type="hidden" value="e_tempissueitems" id="tableName" name="tableName">
              ...
              <table id="saveXStage" class="table table-bordered display full-width" >
                ...
                <td>
                  ...
                  <td><input type="text" class="form-control default_dtpp" name="req_date" id="req_date" required autocomplete="off" ></td>
		  ...
		  if($bomi){
              ...
              <tr id="row<?=$id?>">
                <td width="10%"><?=getField('e_assemblies',array("id"=>$assembly_id),'title');?></td>
                <td width="10%"><?=getField('e_assemblies',array("id"=>$sub_assembly_id),'title');?></td>
                <td width="10%"><?=$item_id?></td>
                <td width="15%"><span onclick="idetailsToolTip(<?=$item_id?>)" onclcik="clearToolTip(<?=$item_id?>)">
                ...
                <button class="btn btn-primary btn-xs" onclick="updateitem(<?=$id?>,'e_tempissueitems',<?=$item_id?>)" > <i class="fa fa-pencil"></i></button>
                 
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  <button class="btn btn-danger btn-xs" onclick="deletBomItem('id',<?=$id?>,'e_tempissueitems')"> <i class="fa fa-trash-o "></i></button>
                  <?php endif?>
                  </td>
              </tr>
              <?php }
...
{   var tableName 	= 'e_tempissue';
	...
	if(confirm("Are you sure to prceed?")){
		...
		function(data)
		{  	
			window.location.href = "<?=base_url()?>Home/tempissue";
		});
	}
}

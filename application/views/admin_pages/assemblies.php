...
<?php 
$pageName = "assemblies";
$calledName = "Assemblies";
$heading = "Assemblies";
$key = "id";
$tableName = "e_assemblies";
$fields = array('Project','Assembly Title','Serial No','Parent Assembly');
$indexes  = array('projectid','title','serialno','parentassembly');
        ...
        <div class="card card-topline-aqua">
          <div class="card-head">
            ...
            <button  style="display:none" type="button" id="btn_popup" value="ppp"></button>
              <button type="button" class="btn btn-round btn-warning" onclick="add('add',<?=$projid?>)" >Add New</button>
            </div>
            <div class="tools">
            ...
            <table id="saveStage" class="table-bordered display full-width" >
              ...
              <tbody>
						<?php foreach ($data as $row):?>
							...
							if($index=='parentassembly' && $row[$index]>0)
							$row[$index]= getField('e_assemblies',array('id'=>$row[$index]),'title');
							else if($index=='parentassembly' && $row[$index]==0)
							$row[$index]= '';
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>

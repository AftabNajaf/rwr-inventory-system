...
<?php 
$pageName = "alloc_details";
$calledName = "";
$heading = "Item Allocation Details";
$key = "id";
$tableName = "e_items_alloc";
$fields = array('Ser #','MIS #','Item','Serial No','Custodian','Instrument','Instrument  No','Quanity');//,'Parent Category'
$indexes  = array('id','item_id','item_details','serial_no','generated_by','instrument','instrument_no','item_quantity');//,'parentid'
              ...
              <tbody>
						<?php $count =0; foreach ($data as $row): $count++;?>
                        	<tr id="row<?=$count?>">
								...
								if(($index=='item_details'))								
									$row[$index]= $row['item_id'].'- '.getField('e_items',array("id"=>$row['item_id']),'item_name');
								if(($index=='generated_by')){
									if($row['instrument']=='mat')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_mat',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									
									if($row['instrument']=='adv')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_adv',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									if($row['instrument']=='tmpissue')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_tempissue',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									if($row['instrument']=='mmov')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_mm',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									if($row['instrument']=='restock')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_restock',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									if($row['instrument']=='release')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_release',array("id"=>$row['instrument_no']),'generated_by')),'name');//
									if($row['instrument']=='tmpissue')
									$row[$index]= @getField('e_admin',array("userid"=>getField('e_tempissue',array("id"=>$row['instrument_no']),'generated_by')),'name');//
								}
									?>
...
function addpo(action,val)
{   var viewType 	= 'Po';
	var tableName 	= 'e_po';
	var key 		= 'id'; 
	$("#exTitle").html(action+' PO');

          ...
          <div class="card-head">
            <div class="tools col-md-1">
              ...
              <?php if($this->session->role=='level1') :?>
                  ...
                  &nbsp;	
              <?php else :?>
              <button type="button" class="btn btn-xs btn-success" onclick="submitBom(<?=$bom['id']?>,'Approved')" > Approved<!--< ?=$bom['approved_status']?>--> </button>
              <br />
              <br />
            ...
            <div class="tools col-md-4"> PROJECT :<br>
              <?=getField('e_projects',array('id'=>$bom['project_id']),'title')?>
            </div>
            <div class="col-md-3">
              <b>BOM No : <?=$bom['id']?></b><br>
...
#saveXStage tr th {padding:0px; text-align:center; height:20px}
            ...
            <form name="myForm" id="myForm" method="post" enctype="multipart/form-data"  onsubmit="return form_validation()">
              ...
              <input type="hidden" value="e_bomitem" id="tableName" name="tableName">
              <input type="hidden" value="id" id="keyIndex" name="keyIndex">
              <input type="hidden" value="0" id="id" name="id">
              <input type="hidden" value="add" id="action" name="action">
              <input type="hidden" class="form-control" name="bom_id" id="bom_id" value="<?=$bom['id']?>" required >
              ...
              <table id="saveXStage" class="table table-bordered display full-width" >
                ...
                <tr>
                  ...
                  <td><select class="form-control" name="sub_assembly_id" id="sub_assembly_id" required >
		  ...
		  if($bomi){
              ...
              <tr id="row<?=$id?>">
                ...
                <td width="20%"><span onclick="idetailsToolTip(<?=$item_id?>)" >
                  <?=getField('e_items',array("id"=>$item_id),'item_name');?>
                  </span></td>
                <td width="10%"><?=$qty_needed?></td>
                <td width="20%"><?=$item_notes?></td>
                <td><button class="btn btn-primary btn-xs" onclick="updateitem(<?=$id?>,'e_bomitem')" > <i class="fa fa-pencil"></i></button>
                
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  <button class="btn btn-danger btn-xs" onclick="deletBomItem('id',<?=$id?>,'e_bomitem')"> <i class="fa fa-trash-o "></i></button>
                  <?php endif?>
                  </td>
...
{   var tableName 	= 'e_bom';
	...
	if(confirm("Are you sure to prceed?")){
		...
		function(data)
		{  	
			window.location.href = "<?=base_url()?>Home/bom";
		});
	}
}

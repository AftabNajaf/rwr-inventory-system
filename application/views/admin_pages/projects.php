...
$heading = "Projects";
$key = "id";
$tableName = "e_projects";
$fields = array('Execution Date','Project Title','Project Status');
$indexes  = array('executiondate','title','status');
            ...
            <table id="saveStage" class="table-bordered display full-width" >
              ...
              <tbody>
						<?php foreach ($data as $row):?>
							...
							if($index=='executiondate' && $row[$index]!='') $row[$index]= date("Y-m-d",strtotime($row[$index]));
							//getField('e_departments',array('id'=>$row[$index]),'title');
								?>
                        			<td><?=$row[$index]?></td>
                        		<?php endforeach ?>
                        
                                <td style="padding: 2px;text-align: center;" width="20%">
                                <button class="btn btn-warning btn-xs" onclick="location.href = '<?=base_url()?>assemblies/<?=$row[$key]?>'" >
                                <i class="fa fa-compass">&nbsp;Assebmlie(s)</i></button>

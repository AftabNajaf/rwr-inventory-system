...
$heading 		= "Delivery Challan - Items";
$key 			= "id";
$tableName 		= "e_items";
      ...
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          <div class="card-head">
            ...
            <div class="tools col-md-3"> Carrier :<br>
               <?=$bom['carrier_name']?>
            </div>
            <div class="tools col-md-5"> Customer :<br>
             <?=getField('e_customers',array('id'=>$bom['customer_id']),'title')?>
            </div>
            <div class="col-md-2">
              <h5> Request No :<br>
                <?=$bom['id']?>
...
#saveXStage tr th {padding:0px; text-align:center; height:20px}
            ...
            <form name="myForm" id="myForm" method="post" enctype="multipart/form-data"  onsubmit="return form_validation()">
              ...
              <input type="hidden" value="e_dcitems" id="tableName" name="tableName">
		  ...
		  if($bomi){
              ...
              <tr id="row<?=$id?>">
                ...
                <td width="15%"><?=$qty_needed?></td>
                <td width="30%"><?=$item_notes?></td>
                <td>
                <button class="btn btn-primary btn-xs" onclick="updateitem(<?=$id?>,'e_dcitems')" > <i class="fa fa-pencil"></i></button>
                 
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  <button class="btn btn-danger btn-xs" onclick="deletBomItem('id',<?=$id?>,'e_dcitems')"> <i class="fa fa-trash-o "></i></button>
                  <?php endif?>
...
{   var tableName 	= 'e_dc';
	...
	if(confirm("Are you sure to prceed?")){
		...
		function(data)
		{  	
			window.location.href = "<?=base_url()?>Home/dc";
		});
	}
}

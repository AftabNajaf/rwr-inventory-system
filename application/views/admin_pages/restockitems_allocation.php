...
$key = "id";
$tableName = "e_items";
$fields = array('Request #','Project','Assembly','Sub-Assembly','Status');//,'Parent Category'
$indexes  = array('id','project_id','assembly_id','sub_assembly_id','approved_status');//,'parentid'
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card card-topline-aqua">
          ...
          Instrument :<br><?=$bom['instrument']." : ".$bom['instrument_no']?> 
          </div>
		  ...
		  <style>#saveXStage tr th {padding:0px; text-align:center; height:20px}</style>    
            <table id="saveXStage" class="table table-bordered display full-width" >
                <tr><th width="10%">Project</th>
                <th width="10%">Assembly</th>
                <th width="10%">Sub-Assembly</th>
                <th width="10%">MIS#</th>
                <th width="20%">Item </th>
                <th width="10%">Qty Restock </th>
                <th width="10%">Qty Returned</th>
                <th width="10%">Item Notes</th>
                <th width="10%">
                <button class="btn btn-info btn-xs" onclick="setStoreRemarks(<?=$bom['id']?>,'restock','e_restockitems','e_restock',<?=$bom['id']?>,'adv_id')" >Mark as Done</button>
		  ...
		  if($bomi){
			  foreach($bomi as $bi){ extract($bi);?>
                <tr id="row<?=$id?>">
                <td width="10%"><?=getField('e_projects',array('id'=>$project_id),'title')?></td>
                ...
                <td width="20%"><span onclick="idetailsToolTip(<?=$item_id?>)" onclick="clearToolTip(<?=$item_id?>)"><?=getField('e_items',array("id"=>$item_id),'item_name');?></span> </td>
                <td width="10%"><?=$qty_restocked?></td>                
                <td width="10%"><?=getField('e_items_alloc',array("instrument_no"=>$bom['id'],'instrument'=>"restock","item_id"=>$item_id),'item_quantity');?></td>
                <td width="10%"><?=$item_notes?></td>
                <td>
                <!--<button class="btn btn-primary btn-xs" onclick="add('edit',<?=$id?>)" >
                ...
                <?php if($this->session->role!='level1' || $bom['approved_status']!='Approved') :?>
                  <!--<button class="btn btn-danger btn-xs" onclick="deletBomItem('id',<?=$id?>,'e_restockitems')"> <i class="fa fa-trash-o "></i></button>-->
                  <?php endif?>
                 <button class="btn btn-success btn-xs" onclick="addEditItemAlloc('add',<?=$bom['id']?>,<?=$item_id?>,'restock')" > <i class="fa fa-gear"></i>Restock</button>
